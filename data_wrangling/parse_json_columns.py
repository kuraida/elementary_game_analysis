...
Here we will identify the columns we want to parse, pull out the relevant info from the json objects, and replace the values in the source dataframe/csv/table with the id reference to cut down on redundancy.
...
def parse_json(in_df, fields):
    ...
    for field in fields:
        ...
        for idx in in_df.index:
            ...
            json_eval = ast.literal_eval(json_string)
        ...
        out_df.to_csv('./data/' + field + '.csv', index=False)
...
def main():
    ...
    input_df = pd.read_csv('./data/game_details.csv')[:20]
    field_list = ['genres']
    parsed_df = parse_json(input_df, ['genres'])
    print(parsed_df['genres'].head())
